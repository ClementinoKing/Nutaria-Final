...
function StockLevels() {
  ...
  const productLookup = useMemo(
    () =>
      products.reduce((accumulator: Record<number, Product>, product: Product) => {
  ...
  const enrichedStockLevels = useMemo((): EnrichedStockLevel[] => {
    return stockLevels.map((entry: StockLevel): EnrichedStockLevel => {
      const product = productLookup[entry.product_id] ?? null
      ...
      const dailyAverageDemand = (product?.target_stock ?? 0) > 0 ? (product?.target_stock ?? 0) / 30 : 0
      const daysOfCover =
