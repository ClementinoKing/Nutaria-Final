...
function Units() {
  ...
  const stats = useMemo(() => {
    ...
    preparedUnits.forEach((unit) => {
      if (unit.createdAtDate) {
        if (!latestUnit || unit.createdAtDate > latestUnit.createdAtDate) {
          latestUnit = unit
        }
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        render: (unit) => formatDate(unit.createdAtDate),
        mobileRender: (unit) => formatDate(unit.createdAtDate),
      },
    ],
    []
  )
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      setUnits((previous) => (data ? [data, ...previous] : previous))
      ...
      toast.error(insertError.message ?? 'Unable to add unit.')
      setIsSubmitting(false)
    }
  }
