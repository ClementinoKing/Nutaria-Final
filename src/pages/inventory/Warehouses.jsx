...
function Warehouses() {
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        mobileRender: (warehouse) => (warehouse.isEnabled ? 'Active' : 'Disabled'),
      ...
      },
      {
        ...
        mobileRender: (warehouse) => warehouse.code ?? '—',
      ...
      },
      {
        ...
        render: (warehouse) => formatDate(warehouse.createdAtDate),
        mobileRender: (warehouse) => formatDate(warehouse.createdAtDate),
      },
    ],
    []
  )
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      setWarehouses((previous) => (data ? [data, ...previous] : previous))
      handleCloseModal()
    } catch (insertError) {
      console.error('Error creating warehouse', insertError)
      toast.error(insertError.message ?? 'Unable to add warehouse.')
      setIsSubmitting(false)
    }
  }
