...
function ProcessView() {
  ...
  const timelineByRunId = useMemo(() => {
    ...
    runs.forEach((run) => {
      ...
      const orderedSteps = [...steps].sort((a, b) => (a.seq ?? 0) - (b.seq ?? 0))

      const timelineItems = orderedSteps.map((step, index) => {
        const processStep = processSteps.get(run.processes?.id ?? -1)?.find((s) => s.id === step.step_id)
        const label = processStep?.step_name ?? step.step_name ?? `Step ${step.seq ?? index + 1}`
  ...
  const columns = [
    {
      ...
      render: (run) => (
        <div>
          <div className="font-medium text-text-dark">{run.supply_batches?.lot_no ?? 'Unknown lot'}</div>
          <div className="text-xs text-text-dark/60">
            {run.supply_batches?.supplies?.doc_no ?? 'No document number'}
          </div>
        </div>
      ),
      mobileRender: (run) => (
    ...
    },
    {
      ...
      render: (run) => (
        <div>
          <div className="text-text-dark font-medium">{run.supply_batches?.products?.name ?? 'Unknown product'}</div>
          <div className="text-xs text-text-dark/60">{run.supply_batches?.products?.sku ?? '—'}</div>
        </div>
      ),
      mobileRender: (run) => (
        <div className="text-right">
    ...
    },
    {
      ...
      render: (run) => (
        <div>
          <div className="text-sm font-medium text-text-dark">{run.processes?.name ?? 'Unknown process'}</div>
          <div className="text-xs text-text-dark/60">{run.processes?.code ?? '—'}</div>
        </div>
      ),
      mobileRender: (run) => (
        <div className="text-right text-sm text-text-dark">{run.processes?.name ?? 'Unknown process'}</div>
      ),
    },
    {
      key: 'status',
      ...
      mobileRender: (run) => run.status ?? 'UNKNOWN',
    },
    {
      key: 'started_at',
      header: 'Started',
      render: (run) => formatDateTime(run.started_at),
      mobileRender: (run) => formatDateTime(run.started_at),
    ...
    },
    {
      key: 'completed_at',
      header: 'Completed',
      render: (run) => formatDateTime(run.completed_at),
      mobileRender: (run) => formatDateTime(run.completed_at),
      cellClassName: 'text-sm text-text-dark/70',
      mobileValueClassName: 'text-sm text-text-dark',
    },
    {
  ...
  const getTimelineAccent = (status) => {
    switch ((status ?? '').toUpperCase()) {
      case 'COMPLETED':
        return {
          ring: 'bg-olive/30',
      ...
      <Card className="bg-white border-olive-light/30">
        ...
        <CardContent>
          ...
          {loading ? (
            ...
            <ResponsiveTable columns={columns} data={runs} rowKey="id" emptyMessage="No process runs yet." />
          )}
        </CardContent>
      </Card>
            ...
            <div className="flex items-center justify-between border-b border-olive-light/20 p-4 sm:p-6">
              <div>
                ...
                <p className="mt-1 text-sm text-text-dark/70">
                  {selectedRun.supply_batches?.lot_no ?? 'Unknown lot'} ·{' '}
                  {selectedRun.supply_batches?.products?.name ?? 'Unknown product'}
                </p>
                <p className="text-xs text-text-dark/50">
                  Process: {selectedRun.processes?.name ?? 'Unknown'} ({selectedRun.processes?.code ?? '—'})
                </p>
              </div>
              <Button
                variant="ghost"
                              ...
                              <div className="flex items-center justify-between gap-3">
                                <div>
                                  ...
                                  <div className="mt-1 text-xs text-text-dark/60">
                                    Started: {formatDateTime(item.started_at)} · Completed: {formatDateTime(item.completed_at)}
                                  </div>
                                </div>
