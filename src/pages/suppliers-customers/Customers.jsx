...
const populateFormFromCustomer = (customer) => {
  if (!customer) {
  ...
  const contacts =
    customer.contacts?.length > 0
      ? customer.contacts.map((contact) => ({
          clientId: contact.id ?? createUniqueId('contact'),
          id: contact.id ?? null,
          name: contact.name ?? '',
          email: contact.email ?? '',
          phone: contact.phone ?? '',
...
function Customers() {
  ...
  const handleContactChange = (clientId, key, value) => {
    setFormData((prev) => ({
      ...prev,
  ...
  const handleRemoveContact = (clientId) => {
    setFormData((prev) => {
  ...
  const validateForm = (data) => {
    ...
    data.contacts.forEach((contact) => {
      const hasAnyValue = ['name', 'email', 'phone', 'role'].some((key) => contact[key]?.trim())
      if (!hasAnyValue) {
        return
      }
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      toast.error(submissionError.message ?? 'Unable to save customer.')
    } finally {
      setIsSubmitting(false)
    }
  }
  ...
  const columns = [
    {
      ...
      render: (customer) => (
        <div className="space-y-0.5">
          <p className="text-sm font-medium text-text-dark">{customer.name}</p>
          <p className="text-xs text-text-dark/60">{customer.email || 'No email'}</p>
        </div>
      ),
    ...
    },
    {
      ...
      mobileRender: (customer) => customer.phone || '—',
    ...
    <PageLayout
      ...
      actions={
        <div className="flex gap-2">
          ...
          <Button size="sm" className="bg-olive hover:bg-olive-dark" onClick={handleOpenModal}>
              ...
              {selectedCustomer ? (
                <>
                  <div>
                    ...
                    <p className="text-base font-medium text-text-dark">{selectedCustomer.name}</p>
                  </div>
                  <div>
                    <p className="text-xs uppercase tracking-wide text-text-dark/60">Main contact</p>
                    <p className="text-sm text-text-dark/80">{selectedCustomer.email || '—'}</p>
                    <p className="text-sm text-text-dark/80">{selectedCustomer.phone || '—'}</p>
                  </div>
                  <div>
                      ...
                      <ul className="space-y-2">
                        {selectedCustomer.contacts.map((contact) => (
                          ...
                          >
                            ...
                            <p className="text-xs text-text-dark/60">{contact.role || 'No role recorded'}</p>
                            <p className="text-xs text-text-dark/60">{contact.email || 'No email provided'}</p>
                            <p className="text-xs text-text-dark/60">{contact.phone || 'No phone provided'}</p>
                          </li>
                        ))}
                      </ul>
                    ) : (
