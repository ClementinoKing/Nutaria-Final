...
function SupplyDetail() {
  ...
  const resolveProfileName = (profileId) => {
    if (!profileId) {
      return null
    }
  ...
  const getUnitMeta = (unitId) => {
    ...
    return unitLookup.get(String(unitId)) ?? null
  }
  ...
  const formatQuantityWithUnit = (value, unitId) => {
  ...
  const summariseQuantityByUnit = (items, key) => {
    ...
    return Array.from(totalsByUnit.entries())
      .map(([unitKey, value]) => {
        const unitMeta = unitLookup.get(unitKey)
  ...
  const lineColumns = [
    ...
    },
    {
      ...
      render: (line) => line.variance_reason || 'On plan',
      mobileRender: (line) => line.variance_reason || 'On plan',
