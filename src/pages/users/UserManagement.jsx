...
function isValidNutariaEmail(email) {
...
function UserManagement() {
  ...
  const filteredUsers = useMemo(() => {
    ...
    return profiles.filter((profile) => {
      ...
      const roleLabel = roleMeta?.label ?? profile.role ?? ''
      return (
        (profile.full_name ?? '').toLowerCase().includes(normalisedSearch) ||
        (profile.email ?? '').toLowerCase().includes(normalisedSearch) ||
  ...
  const handleDelete = async (profile) => {
    const roleMeta = getRoleMeta(profile.role)
    const displayName = profile.full_name || profile.email || profile.auth_user_id
                        ...
                        <td className="px-4 py-3">
                          <div className="flex flex-col">
                              ...
                              <span className="text-xs text-text-dark/60">
                                Auth ID: {profile.auth_user_id}
                              </span>
                          </div>
                        </td>
                          <td className="px-4 py-3">{profile.email ?? '—'}</td>
                          <td className="px-4 py-3">{roleMeta?.label ?? profile.role ?? '—'}</td>
              ...
              <div className="grid gap-4 md:grid-cols-2">
                ...
                {selectedProfile ? (
                  <div className="space-y-2">
                    ...
                    <Input id="auth-id" value={selectedProfile.auth_user_id} disabled />
                  </div>
                ) : (
                <div className="space-y-2">
